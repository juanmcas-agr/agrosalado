-- 027: arregla un bug real que quedó de la migración anterior de
-- Traslado (pedir también rodeo destino, ver 026 y el cambio en
-- stock/js/movimientos.js/config.js). Dos triggers de validar_movimiento
-- / actualizar_rodeo_tras_movimiento todavía asumían que SOLO
-- cambio_rodeo (y cambio_categoria vía Destete) podían traer
-- rodeo_destino_id — bloqueaban cualquier traslado con el error
-- "rodeo_destino_id no corresponde para traslado".
--
-- 1) validar_movimiento(): permite rodeo_destino_id en traslado, y lo
-- exige (igual que ya exige para cambio_rodeo) — tiene que ser distinto
-- del rodeo de origen.
...
-- 2) actualizar_rodeo_tras_movimiento(): con rodeo_destino_id ahora
-- siempre presente en un traslado (el cliente lo exige), el rodeo de
-- origen ya NO se "reubica en el lugar" — las cabezas se acreditan al
-- rodeo destino (existente o recién creado, vía movimiento_lineas, que
-- ya usa coalesce(rodeo_destino_id, rodeo_id) para cualquier tipo). Se
-- deja el update viejo solo como resguardo por si alguna vez llega un
-- traslado sin rodeo_destino_id.
