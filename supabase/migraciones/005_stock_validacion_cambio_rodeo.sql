-- Migración: validación de stock disponible + traslado/reasignación de
-- rodeo (Hacienda M4). Correr completo en el SQL Editor de Supabase.

-- 1. Nueva columna: solo se usa en 'cambio_rodeo' (mover animales de un
-- rodeo a otro sin cambiar establecimiento/categoría/titular).
alter table movimientos add column rodeo_destino_id uuid references rodeos(id);

-- 2. Nuevo tipo de movimiento.
...
-- 3. Vistas: hay que soltarlas en orden (historial y stock_actual dependen
-- de movimiento_lineas/movimientos) antes de poder recrearlas.
...
-- 4. Trigger de validación: agrega las reglas de 'cambio_rodeo' (mismo
-- archivo completo, ya que plpgsql no permite "parchear" una función).
...
-- 5. Trigger nuevo: mantiene rodeos.establecimiento_id/categoria_id al
-- día cuando el rodeo se traslada o cambia de categoría.
...
-- Verificación: el tipo nuevo existe, y las vistas responden.
select id, nombre from tipos_movimiento where id = 'cambio_rodeo';
