// Lógica real de los índices de inflación — separada de
// indices-inflacion.js (el wrapper programado) para poder compartirla con
// actualizar-indices-inflacion.js (SIN schedule, invocable por HTTP para
// probarla a mano). Ver la nota en indices-inflacion.js.
//
// ARS vía IPC INDEC (API pública ArgentinaDatos, que da variación % mensual
// — acá se encadena para construir un índice acumulado) y USD vía CPI de
// EEUU (FRED, serie CPIAUCSL, que ya viene como índice directo).
//
...
async function ejecutarIndicesInflacion() {
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return { ok: false, mensaje: 'Falta configurar SUPABASE_SERVICE_ROLE_KEY en Netlify.' };
  }
  if (!process.env.FRED_API_KEY) {
    return { ok: false, mensaje: 'Falta configurar FRED_API_KEY en Netlify.' };
  }
...
module.exports = { ejecutarIndicesInflacion };
