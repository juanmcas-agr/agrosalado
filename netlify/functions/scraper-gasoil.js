// Gas oil Grado 2: usa el dataset público nacional "Precios en Surtidor"
// (Secretaría de Energía, Resolución 314/2016) como referencia — el
// proveedor real de Juan (Casa Eliceiry, Castelli) hace más de un año que
// no reporta ahí ese precio. Juan confirmó usar como referencia la
// estación YPF de Pilar ("GAS IMPULSO S.A.") o, si deja de reportar, San
// Pedro ("JORGE ALBERTO CASO S.A.") — región Pampeana, horario Diurno.
//
...
// probar el parseo sin esperar al cron, mismo criterio que pizarra.js.
...
exports.handler = async function () {
  ...
  try {
    ...
    if (!encontrado) {
      console.error('No se encontró Gas Oil Grado 2 para ninguna estación de referencia (Pilar/San Pedro).');
      return { statusCode: 200, body: 'Sin datos hoy para ninguna estación de referencia.' };
    }
    await upsertPrecio('gasoil_g2', fechaHoyArt(), encontrado.precio, 'scraper');
    return { statusCode: 200, body: JSON.stringify(encontrado) };
  } catch (error) {
    console.error('Error en scraper-gasoil:', error);
